...
# 实现了一个动态变化rate的损失函数，基于loss_n.py代码
    ...
    def segment_level_loss(self, gt, pred, op='xor', out_size=(160, 160)):
        ...
        for i in range(0, pred_num):
            ...
            if gt_labeled[mask].min() != gt_labeled[mask].max():
                ...
                # 加新rate功能
                new_pic2 = np.zeros(pred.shape, dtype=np.uint8)
                new_pic2[pred_labeled_i] = 1
                rate = xxn_dqn.learn_rate(new_pic2,new_pic)
        ...
        for i in range(0, gt_num):
            ...
            if pred_labeled[mask].min() != pred_labeled[mask].max():
                ...
                # 加新rate功能
                new_pic2 = np.zeros(gt.shape, dtype=np.uint8)
                new_pic2[gt_labeled_i] = 1
                rate = xxn_dqn.learn_rate(new_pic2,new_pic)   
